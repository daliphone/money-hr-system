...
const THEMES = {
  dark: {
    ...
    headerBg: "linear-gradient(135deg,#0d1829,#0a1520)",import React, { useState, useEffect } from "react";
import { Users, Target, MessageSquare, TrendingUp, ChevronRight, ClipboardCheck, BrainCircuit, Award, AlertCircle, Loader2, UserCheck, Trash2, Settings2, PlusCircle, Database } from "lucide-react";
...
export default function SuitabilityAssessment() {
  ...
  async function fetchAI(scores) {
    ...
    try {
      ...
      // 【除錯關鍵】不要直接轉 JSON，先看 GAS 到底吐了什麼鬼東西回來
      const text = await response.text(); 
      let data;
      try {
        data = JSON.parse(text);
      } catch (parseError) {
        throw new Error(`GAS 拒絕連線或回傳了錯誤網頁 (可能是權限未開成所有人)。\n攔截到的內容：${text.substring(0, 60)}...`);
      }
      
      if (data.error) throw new Error(data.error);
      setAiText(data.reply || "無評語資料");
    } catch (e) { 
      setAiText(`⚠️ 系統攔截到錯誤：${e.message}`); 
    }
    setAiLoading(false);
  }
  ...
  function handleAnalyze() {
    ...
    setTimeout(() => {
      try {
        const scores = calcResult(); // 運算分數
        setResult(scores);
        setStep(4); // 強制切換到結果畫面
        fetchAI(scores); // 呼叫後端
        setAnalyzing(false);
      } catch (error) {
        // 【除錯關鍵】如果前端算分數算到當機，強制彈出警告視窗！
        alert("🚨 系統卡住！捕捉到前端運算崩潰：\n" + error.message);
        console.error("崩潰細節:", error);
        setAnalyzing(false);
      }
    }, 800); // 縮短等待時間，加速測試
  }
